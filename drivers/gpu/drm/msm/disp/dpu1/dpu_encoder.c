...
dpu_encoder_dsc_pclk_param_calc(struct msm_display_dsc_config *dsc, u32 width)
{
	...
	if (!dsc || !dsc->drm.slice_width || !dsc->slice_per_pkt) {
		DPU_ERROR("Invalid DSC/slices Slice Witdh %d, per pkt %d\n",
				dsc->drm.slice_width, dsc->slice_per_pkt);
		return;
	}
				 ...
				 struct msm_display_dsc_config *dsc)
{
	...
	for (i = 0; i < num_dsc; i++) {
		...
		if (!hw_pp[i] || !hw_dsc[i]) {
			DPU_ERROR_ENC(dpu_enc, "invalid params for DSC %d\n", i);
			return;
		}
	}
	...
	dpu_encoder_dsc_pipe_cfg(hw_dsc[0], hw_pp[0], dsc,
				 dsc_common_mode, ich_res);
}
